...
class GoogleMap extends React.Component {
  ...
  initMap = () => {
    ...
    this.state.list.map((item) => {
      ...
      marker.addListener("click", function () {
        infoWindow.setContent(`<div class="card" style="width: 18rem;"><img src=${item.imgUrl} class="card-img-top" alt="...">
        <div class="card-body">
          <h5 class="card-title">${item.title}</h5>
          <h6 class="card-text mb-3">Address: ${item.address}</h6>
          <a href="/property-detail/${item._id}" class="btn btn-primary">View Details</a>
        </div>
      </div>`);
  ...
  render() {
    ...
    if (this.state.list.length > 0) {
      renderComponent = (
        <div id="map" className="mt-5" style={{ width, height }}></div>
      );
    } else {
      renderComponent = (
        <div
...
export default connect(mapStateToPropes, actions)(GoogleMap);
